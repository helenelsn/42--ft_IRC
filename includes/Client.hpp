...
/*   Updated: 2024/08/22 12:15:17 by Helene           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    Client::connect_to_s(std::string const& port)
{
    ...
    _client_fd = socket(sa.sin_family, SOCK_STREAM, 0);
    if (_client_fd == -1) {
        fprintf(stderr, "socket fd error: %s\n", strerror(errno));
        // throw exception ;
    } 

    status = connect(_client_fd, (struct sockaddr *)&sa, sizeof sa);
    if (status != 0) {
...
void Client::send_msg()
{
    ...
    int bytes_sent = send(_client_fd, msg.c_str(), msg_size, 0);
    if (bytes_sent == -1)
        ;// exception
...
void Client::receive_msg()
{
    ...
    int bytes_read = 1;
    while (bytes_read >= 0) {
        bytes_read = recv(_client_fd, buffer, BUFSIZ, 0);
        if (bytes_read == 0) {
